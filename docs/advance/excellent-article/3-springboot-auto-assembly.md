...
![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akN9FvT916DA6Uiazd7AJ9D6chwML7lohekeUS2X7kLicnpC96p0toj2rug/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)那metadata是什么呢，可以看到是标注在@SpringBootApplication注解上的DemosbApplication，也就是我们的主配置类Application：

![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akNesTib7YDsmxg53F5x8zchX4ibNibZPJ2qz5q8tV0R1ryF3D6DqG3X8b0w/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)其实就是将主配置类（即@SpringBootApplication标注的类）的所在包及子包里面所有组件扫描加载到Spring容器。因此我们要把DemoApplication放在项目的最高级中（最外层目录）。
...
![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akNIJ5ibA5Xx5sdhSMWtfLsfcib9W2PrbpJdHicqpiabHLExKgKcSymiakhN6Q/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)可以从图中看出AutoConfigurationImportSelector 继承了 DeferredImportSelector 继承了 ImportSelector，ImportSelector有一个方法为：selectImports。将所有需要导入的组件以全类名的方式返回，这些组件就会被添加到容器中。
...
![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akNDJmCmH2R05P4JBsOaFnF8Y3tnt0sFOHOO5mfHdKHxWxJE3vyWcwG2A/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)有了自动配置类，免去了我们手动编写配置注入功能组件等的工作。那是如何获取到这些配置类的呢，看看下面这个方法：
...
![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akNmnZSK7j5eqUa4O8cjCT0pbcLRN5F2lXIN2mlXYpjXdic676FlXASaAg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)接下来看@ComponentScan注解，@ComponentScan(excludeFilters = { @Filter(type = FilterType.CUSTOM, classes = TypeExcludeFilter.class), @Filter(type = FilterType.CUSTOM, classes = AutoConfigurationExcludeFilter.class) })，这个注解就是扫描包，然后放入spring容器。
...
![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akN92R8rsHbWpbxAFSwqdia8FNTI4MC8nFZoOpSjf4WtAice7iaibibFgiaEtQw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)比如Tomcat跟web有关，我们可以看到有个ServletWebServerApplicationContext：
...
![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akN9M0rdnqYTTrMViaNfYVgG0N2dXoM3xP1q8jyuBzRywLHkGlt0rf0e2w/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![](https://mmbiz.qpic.cn/mmbiz_png/JfTPiahTHJhpUFfrddvAHmB3bNFTu6akNf5oUT4DeAQ5yvsNruIOibcJNpfBPVATous50srP31qdtpEZmN4VjTWg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)img
